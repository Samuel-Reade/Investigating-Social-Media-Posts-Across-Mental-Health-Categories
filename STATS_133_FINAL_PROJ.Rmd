...
train <- features[idx, ] %>% select(-post_id)
test  <- features[-idx, ] %>% select(-post_id)
...
m_multinom <- nnet::multinom(label ~ ., data = train, trace = FALSE)
pred_multinom <- predict(m_multinom, newdata = test)
caret::confusionMatrix(pred_multinom, test$label)
...
set.seed(100)
m_rf <- randomForest::randomForest(label ~ ., data = train, ntree = 400, importance = TRUE)
pred_rf <- predict(m_rf, newdata = test)
caret::confusionMatrix(pred_rf, test$label)
